...
# FlashAttention 3 is Hopper-only (H100/H200). On non-Hopper GPUs we fall back
# to torch.nn.functional.scaled_dot_product_attention below.
try:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if flash_attn_3_func is not None:
            ...
            # Fallback for non-Hopper GPUs: shape-shuffle to (B, H, T, D) for SDPA.
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        self.z_loss_coef = h.z_loss_coef
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # PaLM-style auxiliary z-loss: penalize log-partition magnitude for bf16 stability.
        if self.z_loss_coef > 0.0:
            z = flat_logits.logsumexp(dim=-1)
            loss = loss + self.z_loss_coef * (z * z).mean()
        return loss
...
# Per-iteration NS coefficients from Polar Express (Amsel et al., arxiv 2505.16932;
# modded-nanogpt PR #134). Requires 1.02x safety multiplier on input scaling so that
# spectral norm of normalized X is strictly < 1 (otherwise step 0 diverges).
POLAR_EXPRESS_ENABLED = bool(int(os.environ.get('POLAR_EXPRESS', '0')))
POLAR_EXPRESS_COEFS = (
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7) -> Tensor:
    ...
    if transposed:
        X = X.T
    if POLAR_EXPRESS_ENABLED:
        n_coefs = len(POLAR_EXPRESS_COEFS)
        for i in range(steps):
            a, b, c = POLAR_EXPRESS_COEFS[min(i, n_coefs - 1)]
            A = X @ X.T
            B = b * A + c * A @ A
            X = a * X + B @ X
    else:
